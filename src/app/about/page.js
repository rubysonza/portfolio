...
export default function AboutPage() {
    ...
    return (
        <main ref={ref} className='py-[6rem] md:py-[8rem] flex flex-col justify-center items-center'>
            <div className='relative flex justify-center items-center h-auto'>
                <h3 className="text-center text-5xl sm:text-6xl md:text-7xl lg:text-8xl">Get to know me</h3>
            </div>

            <div className="relative h-[100vh] flex flex-col max-w-[1024px]">
                <div className="sticky top-0 flex flex-col justify-center items-center bg-white">
                    <div className="relative flex justify-center items-end min-w-[280px] sm:min-w-[340px] md:min-w-[400px] lg:min-w-[500px]">
                        <div className="absolute top-4 sm:top-12 left-0 right-0 flex flex-col items-start w-60 h-auto pl-3">
                            <p className="flex font-bold text-purple text-4xl sm:text-5xl md:text-6xl lg:text-7xl leading-10 md:leading-15 lg:leading-20">Ruby<br></br>Sonza</p>
                    ...
                    <div className="flex flex-col justify-start items-start gap-20">
                        {about.map((section, index) => (
                            <div key={index} className="w-full">
                                <h4 className="text-xl sm:text-3xl font-bold">{section.title}</h4>

                                {section.type === 'text' && (
                                    <div className="flex flex-col gap-4">
                                        {section.content.map((paragraph, pIndex) => (
                                            <p key={pIndex} className="font-redditMono text-sm sm:text-base leading-9">{paragraph}</p>
                                        ))}
                                    </div>
                                )}
                                        ...
                                        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 place-items-center gap-5">
                                            {section.content.map((tool) => (
                                                <div key={tool.name} className="flex flex-col justify-center items-center gap-2">
                                                    ...
                                                    <span className="font-redditMono text-xs">{tool.name}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
